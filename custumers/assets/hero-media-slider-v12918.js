/* MR BAR v1.29.18 — Customer Hero transition stability */
(function(){
  'use strict';
  ...
  function init(root){
    ...
    function stopMedia(slide){
      if(!slide)return;
      const pending=retireTimers.get(slide);
      if(pending){clearTimeout(pending);retireTimers.delete(slide);}
    ...
    function retireMedia(slide){
      if(!slide)return;
      const pending=retireTimers.get(slide);
      if(pending)clearTimeout(pending);
      const id=window.setTimeout(()=>{
        retireTimers.delete(slide);
        if(!slide.classList.contains('is-active'))stopMedia(slide);
      },720);
      retireTimers.set(slide,id);
    }
