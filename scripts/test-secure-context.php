...
assert_secure(mrbar_force_staff_https_enabled(['HTTP_HOST'=>'mrbarsupport.com']),'production host must default to strict HTTPS after SSL recovery');
assert_secure(mrbar_force_staff_https_enabled(['HTTP_HOST'=>'www.mrbarsupport.com']),'www production host must default to strict HTTPS');
assert_secure(!mrbar_force_staff_https_enabled(['HTTP_HOST'=>'nigiwaigroup.com']),'legacy/test host must not be forced by default');
assert_secure(!mrbar_force_staff_https_enabled(['HTTP_HOST'=>'mrbarsupport.com','MRBAR_FORCE_STAFF_HTTPS'=>'0']),'explicit environment override can temporarily disable production HTTPS redirect');
