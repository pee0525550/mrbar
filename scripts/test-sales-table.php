...
check($d['tables'][0]['status']==='occupied','open attribution marks shared table occupied');
$closed=st_apply($d,['action'=>'close','table_id'=>1,'session_id'=>2,'revision'=>1,'receipts'=>'RC004'],11);
check($closed['tables'][0]['status']==='available','closing last round returns shared table to available');
$busy=$d;$busy['checkins']=[['table_id'=>1,'status'=>'waiting']];
$busy=st_apply($busy,['action'=>'close','table_id'=>1,'session_id'=>2,'revision'=>1,'receipts'=>'RC004'],11);
check($busy['tables'][0]['status']==='occupied','active check-in prevents freeing table');
$busy=$d;$busy['floor_service_sessions']=[['table_id'=>1,'ended_at'=>null]];
$busy=st_apply($busy,['action'=>'close','table_id'=>1,'session_id'=>2,'revision'=>1,'receipts'=>'RC004'],11);
check($busy['tables'][0]['status']==='occupied','active floor service prevents freeing table');
$blocked=$d;$blocked['tables'][0]['status']='blocked';
$blocked=st_apply($blocked,['action'=>'close','table_id'=>1,'session_id'=>2,'revision'=>1,'receipts'=>'RC004'],11);
check($blocked['tables'][0]['status']==='blocked','closing preserves blocked table');
$override=$d;$override['tables'][0]['status']='available';$override=db_sync_sales_table_status($override);
check($override['tables'][0]['status']==='occupied','other screens cannot free an active Sales round');
require_once __DIR__.'/../app/floor-plan.php';
$item=['type'=>'table','table_id'=>1,'reservable'=>1];
check(fp_table_state($d,$item)['status']==='occupied'&&!fp_table_state($d,$item)['can_book'],'public floor map prevents booking occupied Sales table');
check(fp_table_state($closed,$item)['can_book'],'public floor map reopens booking after closure');
...
$raw['branch_data']['2']['sales_table_sessions'][]=['id'=>999,'branch_id'=>3,'table_id'=>77,'table_code'=>'FOREIGN','sales_id'=>77,'sales_label'=>'Foreign Sale','status'=>'closed','opened_at'=>date('c'),'closed_at'=>date('c'),'receipts'=>['SAME-RECEIPT'],'match_status'=>'pending','revision'=>1];
check(count(db_branch_view($raw,2)['sales_table_sessions'])===2,'branch view filters an explicitly foreign Sales-table session from a contaminated bucket');
$scopeFixture=['_branch_context'=>['id'=>2],'sales_table_sessions'=>[['id'=>1,'branch_id'=>3,'table_id'=>7,'status'=>'closed','receipts'=>['SAME-RECEIPT']]]];
check(st_receipts($scopeFixture,'SAME-RECEIPT')===['SAME-RECEIPT'],'the same receipt number is allowed when the existing receipt belongs to another branch');
$pub=['_branch_context'=>['id'=>2],'settings'=>['sales_table_qr_secret'=>'test-secret'],'employees'=>[['id'=>20,'active'=>1,'position'=>'sales','name'=>'Sale A','code'=>'S01']],'tables'=>[['id'=>1,'active'=>1,'code'=>'T01','status'=>'available']],'sales_table_sessions'=>[],'checkins'=>[],'floor_service_sessions'=>[],'audit'=>[]];$token=st_public_token($pub,'Priest',1);check(st_public_token_valid($pub,'Priest',1,$token),'signed public QR token validates for its table');check(!st_public_token_valid($pub,'Priest',2,$token),'signed public QR token cannot be reused for another table');$pub=st_apply_public($pub,['action'=>'open','table_id'=>1,'sales_id'=>20],'QR Public');check($pub['sales_table_sessions'][0]['opened_by']===0&&$pub['tables'][0]['status']==='occupied','public QR can open table without login actor');rejected(fn()=>st_apply_public($pub,['action'=>'transfer','table_id'=>1,'session_id'=>1,'revision'=>1,'sales_id'=>20],'QR Public'),'public QR cannot use manager-only actions');
