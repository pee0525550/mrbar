...
if($_SERVER['REQUEST_METHOD']==='POST'){csrf_check();try{
    ...
    $detail=trim((string)($_POST['report_detail']??''));$pairId=bin2hex(random_bytes(8));$storedFiles=[];
    try{
        $storedFiles['bill_detail']=posi_inbox_store_upload($_FILES['bill_file']??[]);
        $storedFiles['menu_sales']=posi_inbox_store_upload($_FILES['menu_file']??[]);
        foreach($storedFiles as $kind=>$stored){$parsed=posi_parse_file(posi_inbox_dir().'/'.$stored['stored_name'],$stored['ext'],20);$map=posi_guess_columns($parsed['headers']);if($kind==='bill_detail'&&(($map['ref']??-1)<0||($map['sales']??-1)<0||($map['date']??-1)<0))throw new RuntimeException('ไฟล์ที่ 1 ต้องมีเลขบิล วันที่/เวลา และยอดขาย');if($kind==='menu_sales'&&(($map['item']??-1)<0||($map['qty']??-1)<0||($map['sales']??-1)<0))throw new RuntimeException('ไฟล์ที่ 2 ต้องมีชื่อเมนู จำนวนขาย และยอดขาย');}
        db_mutate(function($d)use($storedFiles,$title,$detail,$dateFrom,$dateTo,$uid,$pairId){
            if(!isset($d['pos_upload_inbox'])||!is_array($d['pos_upload_inbox']))$d['pos_upload_inbox']=[];
            foreach($storedFiles as $kind=>$stored){
                foreach($d['pos_upload_inbox'] as $existing)if(($existing['sha256']??'')===$stored['sha256']&&(string)($existing['period_from']??'')===$dateFrom&&(string)($existing['period_to']??'')===$dateTo&&in_array((string)($existing['status']??''),['uploaded','processing','completed'],true))throw new RuntimeException(posi_bill_report_label($kind).' ของรอบวันที่นี้มีอยู่ในระบบแล้ว');
                $entry=array_replace($stored,['id'=>next_id($d['pos_upload_inbox']),'upload_pair_id'=>$pairId,'report_kind'=>$kind,'title'=>$title.' · '.posi_bill_report_label($kind),'detail'=>$detail,'period_mode'=>'custom','period_from'=>$dateFrom,'period_to'=>$dateTo,'status'=>'uploaded','uploaded_at'=>date('c'),'uploaded_by'=>$uid,'processed_at'=>'','processed_by'=>null,'batch_id'=>null,'error'=>'']);
                $d['pos_upload_inbox'][]=$entry;$d['audit'][]=['at'=>date('c'),'action'=>'pos_file_uploaded_to_inbox','inbox_id'=>$entry['id'],'report_kind'=>$kind,'upload_pair_id'=>$pairId,'period_from'=>$dateFrom,'period_to'=>$dateTo,'by'=>$uid];
            }return $d;
        });
    }catch(Throwable $uploadError){foreach($storedFiles as $stored){$path=posi_inbox_dir().'/'.basename((string)$stored['stored_name']);if(is_file($path))@unlink($path);}throw $uploadError;}
    $msg='Import ไฟล์ POS ทั้ง 2 ประเภทเข้า Server สำเร็จแล้ว';
}catch(Throwable $e){$err=$e->getMessage();}}
?><!doctype html><html lang="th"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MR BAR — Import File จาก POS</title><link rel="stylesheet" href="assets/admin.css?v=1240"><link rel="stylesheet" href="assets/admin-v14.css?v=1240"><link rel="stylesheet" href="assets/pos-incentive-v1298.css?v=1298"><link rel="stylesheet" href="assets/pos-upload-inbox-v1390.css?v=1437"><link rel="stylesheet" href="assets/pos-wizard-strict-v1410.css?v=1410"><link rel="stylesheet" href="assets/pos-suite.css?v=1470"></head><body class="admin-v14-page posi-page"><?php require_once __DIR__.'/app/admin-nav.php';echo admin_sidebar('incentive',$u);?>
...
<section class="posi-card posi-import-card"><div class="posi-card-head row"><div><small>IMPORT 2 POS REPORTS</small><h2>อัปโหลดรายงานจาก POS จำนวน 2 ไฟล์</h2><p>ทั้งสองไฟล์ต้องเป็นรอบวันที่เดียวกัน ระบบจะแยกข้อมูลบิลออกจากข้อมูลเมนูอัตโนมัติ</p></div><span class="posi-format-badge">XLSX / CSV</span></div>
...
<div class="posi-dual-files"><label class="posi-inbox-file"><span class="upload-icon">1</span><div><b>Report การขายตามรายละเอียดบิล *</b><small>ต้องมีเลขบิล วันที่/เวลา และยอดขาย · ใช้จับคู่รอบเปิดโต๊ะ Sales</small></div><input type="file" name="bill_file" accept=".xlsx,.csv" required></label><label class="posi-inbox-file"><span class="upload-icon">2</span><div><b>Report การขายตามเมนู *</b><small>ต้องมีชื่อเมนู จำนวนขาย และยอดขาย · ใช้คำนวณค่าดื่ม/ค่าคอม</small></div><input type="file" name="menu_file" accept=".xlsx,.csv" required></label></div>
<button>ตรวจและ Import ทั้ง 2 ไฟล์เข้า Server</button></form></section>
