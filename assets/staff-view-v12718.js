(()=>{
 const sel=document.getElementById('employeeSelect'),frame=document.getElementById('prPreviewFrame'),stage=document.querySelector('[data-pv-stage]');
 if(sel)sel.addEventListener('change',()=>{const u=new URL(location.href);u.searchParams.delete('pr_id');u.searchParams.set('employee_id',sel.value);location.href=u.toString();});
