...
$branch=$found['branch'];$u=current_user();
...
$d=db_branch_view($raw,(int)$branch['id']);
...
$err='';$url='sales-table.php?'.http_build_query(['public_branch'=>$slug,'table_id'=>$tableId]);
if($_SERVER['REQUEST_METHOD']==='POST'){
 csrf_check();
 try{
  if((string)($_POST['public_branch']??'')!==$slug||(int)($_POST['table_id']??0)!==$tableId)throw new RuntimeException('ข้อมูล QR ไม่ตรงกัน');
  $input=$_POST;$uid=(int)$u['id'];$bid=(int)$branch['id'];
  db_mutate_global(function($raw)use($input,$uid,$bid){
   $view=db_branch_view($raw,$bid);$view=st_apply($view,$input,$uid);
   return db_merge_branch_view($raw,$view,$bid);
  });
...
$open=st_open_session($d,$tableId);$canManage=user_can($u,'sales_sessions.manage',$d);
$history=array_values(array_filter($d['sales_table_sessions']??[],fn($s)=>$tableId===0||(int)$s['table_id']===$tableId));$history=array_reverse($history);
function st_fields($slug,$tableId,$s=null){?><input type="hidden" name="csrf" value="<?=h(csrf_token())?>"><input type="hidden" name="public_branch" value="<?=h($slug)?>"><input type="hidden" name="table_id" value="<?=$tableId?>"><?php if($s):?><input type="hidden" name="session_id" value="<?=(int)$s['id']?>"><input type="hidden" name="revision" value="<?=(int)$s['revision']?>"><?php endif;}
function st_cards($sales,$d){?><div class="sales-cards"><?php foreach($sales as $e):?><label class="sales-card"><input type="radio" name="sales_id" value="<?=(int)$e['id']?>" required><?php $photo=!empty($e['profile']['profile_photo'])?'sales-table.php?'.http_build_query(['public_branch'=>(string)($_GET['public_branch']??''),'photo'=>(int)$e['id']]):'';if($photo):?><img src="<?=h($photo)?>" alt=""><?php endif;?><b><?=h(st_label($e))?></b></label><?php endforeach;?></div><?php }
?>
...
<form method="post" action="<?=h($url)?>"><?php st_fields($slug,$tableId,$open);?><input type="hidden" name="action" value="close"><label>หมายเลขใบเสร็จ (หนึ่งใบต่อบรรทัด)<textarea name="receipts" rows="4" required maxlength="2500" placeholder="RC-001&#10;RC-002"></textarea></label><p>ปิดแล้วจะรอจับคู่กับรายงานใบเสร็จ POS ยังไม่ถือเป็นยอดขายที่ยืนยัน</p><button>ยืนยันปิดรอบและผูกใบเสร็จ</button></form></section>
<?php endif;?><?php elseif($tableId>0):?><p class="error">ไม่พบโต๊ะนี้ในร้าน</p><?php endif;?>
<section><h2>ประวัติรอบโต๊ะ</h2><div class="scroll"><table><thead><tr><th>โต๊ะ / Sales เจ้าของยอด</th><th>เปิด / ผู้บันทึก</th><th>ปิด / ผู้บันทึก</th><th>ใบเสร็จ</th><th>สถานะยอด POS</th></tr></thead><tbody>
...
<?php if($canManage&&$tableId>0):?><tr><td colspan="5"><details><summary>แก้ไขโดยผู้ดูแล · ต้องระบุเหตุผล</summary><form method="post" action="<?=h($url)?>"><?php st_fields($slug,$tableId,$s);?><input type="hidden" name="action" value="transfer"><label>เปลี่ยน Sales เจ้าของยอด<select name="sales_id" required><?php foreach($sales as $e):?><option value="<?=(int)$e['id']?>" <?=(int)$s['sales_id']===(int)$e['id']?'selected':''?>><?=h(st_label($e))?></option><?php endforeach;?></select></label><label>เหตุผล<input name="reason" required maxlength="500"></label><button>บันทึกการเปลี่ยนเจ้าของยอด</button></form>
<?php if($s['status']==='closed'):?><form method="post" action="<?=h($url)?>"><?php st_fields($slug,$tableId,$s);?><input type="hidden" name="action" value="edit_receipts"><label>แก้เลขใบเสร็จ<textarea name="receipts" required><?=h(implode("\n",$s['receipts']))?></textarea></label><label>เหตุผล<input name="reason" required maxlength="500"></label><button>บันทึกใบเสร็จที่แก้ไข</button></form><?php endif;?></details></td></tr><?php endif;?>
<?php endforeach;?></tbody></table></div><p>แสดง 100 รอบล่าสุด · ประวัติการแก้ไขเก็บใน Audit Log</p></section>
</main></body></html>
