...
function mrbar_require_https(string $fallbackUri='/',bool $allowAdminPreview=false): void {
    if(mrbar_request_is_https())return;
    if($allowAdminPreview&&mrbar_admin_preview_requested()&&!mrbar_force_staff_https_enabled())return;
    if(!mrbar_force_staff_https_enabled()){
        header('X-MR-BAR-Secure-Context: http-fallback');
        return;
    }
    $host=strtolower(preg_replace('/:\\d+$/','',mrbar_safe_request_host())??'');
    if(!in_array($host,['mrbarsupport.com','www.mrbarsupport.com'],true))$host='mrbarsupport.com';
    header('Location: https://'.$host.mrbar_safe_request_uri($fallbackUri),true,302);
    exit;
}
