...
$matchedSalesSessions=array_values(array_filter($d['sales_table_sessions']??[],fn($s)=>in_array((string)($s['match_status']??''),['matched','partial'],true)));$matchedSalesTotal=array_sum(array_map(fn($s)=>(float)($s['matched_net_sales']??0),$matchedSalesSessions));
?>
<!doctype html><html lang="th"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MR BAR — <?=h($title)?></title><link rel="stylesheet" href="assets/admin.css?v=1240"><link rel="stylesheet" href="assets/admin-v14.css?v=1240"><link rel="stylesheet" href="assets/pos-incentive-v1298.css?v=1298"><link rel="stylesheet" href="assets/pos-suite.css?v=1470"></head><body class="admin-v14-page posi-page"><?php require_once __DIR__.'/admin-nav.php';echo admin_sidebar('incentive',$u);?><main class="posi-shell">
...
<?php if($posCore==='commission'):?><section><h2>ยอดเชียร์ลูกค้าของ Sales</h2><p>จับคู่จากเลขใบเสร็จในรอบเปิดโต๊ะกับ Report รายละเอียดบิลแล้ว <?=count($matchedSalesSessions)?> รอบ · ยอดขายที่จับคู่ได้ <?=number_format($matchedSalesTotal,2)?> บาท ยอดนี้แสดงเพื่อประกอบการตั้งกติกาและยังไม่รวมเป็นค่าคอมจนกว่าจะกำหนดอัตรา</p><a href="sales-table.php?public_branch=<?=h(rawurlencode((string)(branch_current($d)['slug']??'')))?>">ตรวจรอบโต๊ะและใบเสร็จ →</a></section><?php endif;?>
